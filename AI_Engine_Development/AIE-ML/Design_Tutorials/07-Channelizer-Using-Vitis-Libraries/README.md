...
Vitis Libraries introduced two new IP elements to simplify building Channelizers.
...
The focus of this tutorial is to show how these IP blocks can be leveraged to build high performance channelizers using a combination of AIE-ML and programmable logic (PL) resources in AMD Versal™ adaptive SoC devices.
...
The polyphase channelizer [[1]] down-converts simultaneously a set of frequency-division multiplexed (FDM) channels carried in a single data stream using an efficient approach based on digital signal processing.
Channelizer use is ubiquitous in many wireless communications systems.
Channelizer sampling rates increase steadily as capabilities of RF-DAC and RF-ADC technology advance, making them challenging to implement in high-speed reconfigurable devices such as field programmable gate arrays (FPGAs).

![figure1](images/channelizer_diagram.png)

A 1D IFFT may be implemented using a 2D IFFT algorithm with higher efficiency overall in cases of larger point size and SSR > 1 regime. This requires resources that span AIE and PL. 
...
This involves analyzing the polyphase channelizer algorithm characteristics and its functional blocks to identify which should be implemented in AI Engines versus PL and establish a data flow with sufficient bandwidth to support the required computations.
...
The approach we shall take in this tutorial is to analyze compute, storage and I/O bandwidth requirements for the filterbank and IFFT to understand how many AI Engine tiles we expect to use.
...
The filterbank will require storage for the filter coefficients and state, requiring 8 Bytes and 4 Bytes per coefficient or sample.
...
Some storage needs to be reserved for sysmem, which is required by the processor to store stack & heap.

This leads to a solution which requires ~18 tiles for the filterbank. Rounding up to a power of 2 will result in a simpler PL solution to avoid managing state.
...
Inspecting vitis_analyzer, we can see that the design uses more tiles than expected (64 vs 32 predicted).
...
Is it possible to trade-off throughput for storage? Yes.

#### Filterbank Library Optimization
The following approach may be used to tradeoff throughput for storage:
...
Inspecting vitis_analyzer, we can see that our resource count dropped to 32 tiles with a throughput = 4096/1.837us = 2300 MSPS.
...
In this tutorial, we explore the use of 2D IFFT IP to implement a 4K-pt IFFT @ 2 GSPS. Resources will span AIE + PL.
We will need to characterize a single instance of the IP and measure throughput to understand how many instances we need to meet performance.

![figure10](images/2D_4k_IFFT.png)

#### IFFT-2D Library Characterization

The first step is to characterize the 2D IFFT AI Engine IP, i.e. vss_fft_ifft_1d_graph, to understand the optimal configuration to meet our requirements.

We can instantiate vss_fft_ifft_1d_graph based on configuration below. The main choice to be made as part of this exercise is what TP_SSR is sufficient to meet our throughput requirement. We will begin assuming TP_SSR=1 and adjust as needed.
...
While an SSR=5 should be sufficient from a resource count perspective, using a SSR that is a power of 2 will simplify the overall design and allow direct mapping of TDM FIR outputs into 2D IFFT input.
For this reason, we will proceed with SSR=8.
...
Inspecting vitis_analyzer, we observe a resource count of 24 AIE-ML tiles.
Achieved throughput for:
* Front 64-point IFFT + point-wise twiddle multiplication = 3520 MSPS
* Back 64-pint IFFT = 4210 MSPS

![figure12](images/ifft4096_2d_array_view.png)

For the transpose blocks, those can be implemented in PL using BRAMs. The PL is assumed to be clocked at 312.5 MHz.
A single BRAM stores up to 36 Kbits, and can be configured as 512 x 72 bits with one write and one read ports.
For more information on BRAMS, refer to [Versal ACAP Memory Resources Architecture Manual](https://docs.amd.com/r/en-US/am007-versal-memory/Block-RAM?tocId=4P4QDFML7TiXdlJaQlcRog).
...
- TDM FIR shall use 32 AI Engine tiles with 32 IO streams
- The 4k-pt IFFT will be implemented using 2D architecture with resources split between AI Engine (compute) and PL (data transpose).
- From a bandwidth perspective, the design requires 2 input and 4 output streams.
- Custom HLS blocks (merge and split) built to manage connectivity between the IPs.
...
- Reduce the number of AIE-ML tiles used by 2D FFT IP from 16 to 10 (by changing TP_SSR from 8 to 5). This will require increased complexity of the HLS managing the connectivity from the TDM FIR output to the 2D FFT input due to state handling.
- Optimize implementation of HLS transpose blocks to avoid use of ping-pong buffers. This should half the number of BRAMs needed.
...
This will take about 90 minutes to run. The build process will generate a folder ```package``` containing all the files required for hardware emulation.
This can be run as shown below.
An optional `-g` can be applied to the ```launch_hw_emu.sh``` command to launch the Vivado waveform GUI to observe the top-level AXI signal ports in the design.

```
[shell]% cd <path-to-design>/package
[shell]% ./launch_hw_emu.sh -g -run-app embedded_exec.sh
```
After hardware emulation run is complete, the following will be displayed on the terminal.

![figure15](images/channelizer_hw_emu.png)

### Hardware

The channelizer design can be built for the VEK280 board using the Makefile as follows:
...
The build process will generate the SD card image in the ```package/sd_card``` folder.
