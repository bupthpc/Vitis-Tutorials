...
void conv2d_w5A::grab_wts( input_async_buffer<bfloat16>& wts_i )
{
  if (state == 0) {
    wts_i.acquire();
    state = 1;
...
void conv2d_w5A::filter_3x3( input_buffer<bfloat16>& ifm_i,
...
inline __attribute__((noinline))
void conv2d_w5A::pass_outputs( output_stream<bfloat16>* pass_o )
{
...
void conv2d_w5A::run( input_buffer<bfloat16>& ifm_i,
                      input_async_buffer<bfloat16>& wts_i,
