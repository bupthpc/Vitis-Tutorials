...
conv2d_w5D::conv2d_w5D( void )
{
  aie::set_rounding(aie::rounding_mode::symmetric_inf);
  aie::set_saturation(aie::saturation_mode::saturate);
  state = 0;
  auto itw = aie::begin_vector<16>(data3);
...
void conv2d_w5D::grab_wts( input_async_buffer<bfloat16>& wts_i )
{
  if (state == 0) {
    wts_i.acquire();
    state = 1;
...
void conv2d_w5D::filter_3x3( input_buffer<bfloat16>& ifm_i, input_async_buffer<bfloat16>& wts_i )
{
...
inline __attribute__((noinline))
void conv2d_w5D::pass_outputs( input_stream<bfloat16>* pass_i, output_buffer<bfloat16>& ofm_o )
{
  // Collect 1st block:
  auto itw = aie::begin_vector<16>(data0);
  for (unsigned ii=0; ii < 768/16; ii++)
    chess_prepare_for_pipelining
  {
    *itw++ = readincr_v<16>(pass_i);
  }
  // Collect 2nd block:
  itw = aie::begin_vector<16>(data1);
  for (unsigned ii=0; ii < 768/16; ii++)
    chess_prepare_for_pipelining
  {
    *itw++ = readincr_v<16>(pass_i);
  ...
  // Collect 3rd block:
  itw = aie::begin_vector<16>(data2);
  for (unsigned ii=0; ii < 768/16; ii++)
    chess_prepare_for_pipelining
  {
    *itw++ = readincr_v<16>(pass_i);
  }
  // Interleave in proper order:
  itw = aie::begin_vector<16>(ofm_o);
  auto itr0 = aie::begin_vector<16>(data0);
  auto itr1 = aie::begin_vector<16>(data1);
  auto itr2 = aie::begin_vector<16>(data2);
  auto itr3 = aie::begin_vector<16>(data3);
  itw = aie::begin_vector<16>(ofm_o);
  for (unsigned rr=0; rr < 3; rr++)
    chess_prepare_for_pipelining
  {
    for (unsigned ii=0; ii < 32*8/16; ii++)
      chess_prepare_for_pipelining
    {
      *itw++ = *itr0++;
    }
...
void conv2d_w5D::run( input_buffer<bfloat16>& ifm_i,
