...
void conv2d_w5C::grab_wts( input_async_buffer<bfloat16>& wts_i )
{
  if (state == 0) {
    wts_i.acquire();
    state = 1;
...
void conv2d_w5C::filter_3x3( input_buffer<bfloat16>& ifm_i, input_async_buffer<bfloat16>& wts_i )
{
...
inline __attribute__((noinline))
void conv2d_w5C::pass_outputs( input_stream<bfloat16>* pass_i, output_stream<bfloat16>* pass_o )
{
  auto itr = aie::begin_vector<16>(data);
  for (unsigned ii=0; ii < 2*768/16; ii++)
    chess_prepare_for_pipelining
  {
...
void conv2d_w5C::run( input_buffer<bfloat16>& ifm_i,
